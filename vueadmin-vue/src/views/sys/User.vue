  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
          ...
          align="center">
        <template slot-scope="scope">
          <el-tag size="small" type="info" v-for="(item,index) in scope.row.sysRoles" :key="index">{{item.name}}</el-tag>
        </template>
      </el-table-column>
  ...
  //   },
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.post('/sys/user/' + (this.editForm.id?'update' : 'save'), this.editForm)
              .then(res => {
                this.$message({
                  showClose: true,
                  message: '恭喜你，操作成功',
    ...
    submitRoleHandle(formName) {
      ...
      console.log(roleIds)
      this.$http.post('/sys/user/role/' + this.roleForm.id, roleIds).then(res => {
        this.$message({
          showClose: true,
          message: '恭喜你，操作成功',
          type: 'success',
          // onClose:() => {    //弹窗宽比后，再重新获取用户列表数据，不建议使用，因为这样数据显示比较慢，把获取用户列表方法写在外面
          //   this.getUserList()
          // }
        });
        this.getUserList()
    ...
    repassHandle(id, username) {
      this.$confirm('将重置用户【' + username + '】的密码, 是否继续?', '提示', {
        ...
        this.$http.post("/sys/user/repass", id).then(res => {
          this.$message({
            showClose: true,
            message: '恭喜你，操作成功',
            type: 'success',
