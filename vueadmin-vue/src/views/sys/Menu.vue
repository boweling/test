<template>
  <div>
    ...
    <el-table
        ...
        border
        stripe
        :tree-props="{children: 'children', hasChildren: 'hasChildren'}">  <!--stripe属性，可以设置斑马纹表格-->
      <el-table-column
          prop="name"
          label="名称"
          sortable
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.post('/sys/menu/' + (this.editForm.id ? 'update' : 'save'), this.editForm)
              .then(res => {
                this.$message({
                  showClose: true,
                  message: '恭喜你，操作成功',
    ...
    editHandle(id) {
      this.$http.get('/sys/menu/info/' + id).then(res => {
    ...
    resetForm(formName) {
      ...
      this.dialogVisible = false    //在弹出的编辑对话框里，点击重置，对话框消失
      this.editForm = {}
    },
    handleClose() {
